...
# cd ~/kali-nethunter
#
# git clone https://github.com/bnb32/shield-tablet-kernel.git ./shield-tablet-kernel
#
######## Local Repo ##########
...
echo ""
echo -e "\e[31m	[1] Build for Shield Tablet \e[0m"
echo ""
...
f_interface_shield(){
echo -e "\e[31m	------------------------- Shield -----------------------\e[0m"
echo ""
echo "  [1] Build All - Kali rootfs and Kernel (Android 5.1.1+)"
echo "  [2] Build Kernel Only (Android 5.1.1+)"
echo "  [0] Exit to Main Menu"
echo ""
echo ""
# wait for character input

read -p "Choice: " shield_menuchoice

case $shield_menuchoice in

1) d_clear; f_rootfs ; f_flashzip ; f_shield_kernel ; f_zip_save ; f_zip_kernel_save ; f_rom_build ;;
2) d_clear; f_shield_kernel ; f_zip_kernel_save ;;
...
base="kali-menu kali-defaults initramfs-tools usbutils openjdk-7-jre mlocate android-tools-adb android-tools-fastboot"
...
mana="python-twisted python-dnspython libnl-3-200 libnl-3-dev libssl-dev sslsplit python-pcapy tinyproxy isc-dhcp-server rfkill mana-toolkit"
...
debootstrap --foreign --arch $architecture kali-current kali-$architecture http://http.kali.org/kali
...
rm -f /debconf.set
safe-apt-get update
safe-apt-get -y install git-core binutils ca-certificates initramfs-tools u-boot-tools
...
case $1 in
  ...
  kernel)
    case $2 in
      ...
      shield)
        ...
        exportdir=${exportdir%/}
        nightlytype=kernel
        device=shield-tablet
        f_check_version_noui
        f_shield_kernel
        f_zip_kernel_save
        cd ${basedir}
        mkdir -p $exportdir/Kernels/shield-tablet
        mv kernel-kali-$VERSION.zip $exportdir/Kernels/shield-tablet/Kernel-$device-$VERSION.zip
        mv kernel-kali-$VERSION.sha1sum $exportdir/Kernels/shield-tablet/Kernel-$device-$VERSION.sha1sum
        rm -rf ${basedir}
        exit;;
      *)
        d_clear
  ...
  help)
    ...
    echo "   [shield] -----------Nvidia Shield Tablet"
    echo ""
    echo "Directory:"
